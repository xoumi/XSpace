...
const Nav = function (): React.ReactElement {
      ...
      >
        {routes.map((route, i) => (
          <li className={getClassName(route)} key={route.path}>
            <div className='background' ref={function (el) { bgRefs.current[i] = el !== null ? el : null }} />
            <Link href={route.path} prefetch={false}>
              <a>
                <h2 className='text-lg' ref={function (el) { titleRefs.current[i] = el !== null ? el : null }}>{route.title}</h2>
              </a>
            </Link>
          </li>
        ))}
